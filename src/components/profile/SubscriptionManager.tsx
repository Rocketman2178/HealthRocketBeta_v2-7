...
import { X, Shield, Users, Building2, Rocket, Gift, CreditCard } from 'lucide-react';
...
export function SubscriptionManager({ onClose, userData }: SubscriptionManagerProps) {
  ...
  const handlePlanClick = async (plan: Plan | null) => {
    ...
    try {
      ...
      const result = await createSubscription(plan.price_id, plan.trialDays || 0, plan.promoCode || false);
    ...
    <div className="w-full max-w-4xl bg-gray-800 rounded-lg my-8 max-h-[90vh] overflow-y-auto relative z-[201]">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <div className="flex items-center gap-1 text-orange-500">
              <Shield size={14} />
              <span className="text-sm font-medium">{userData?.plan || 'Free Plan'}</span>
            </div>
            {daysLeft && (
      ...
      <div className="p-6 overflow-y-auto">
        {activeTab === 'plans' ? (
          <div className="space-y-6">
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-white mb-2">Available Plans</h3>
              ...
              {daysLeft && userData?.plan === 'Pro Plan' && (
                <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-lg mb-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-white font-medium mb-1">Pro Plan Trial</h4>
                      <p className="text-sm text-gray-300 mb-2">
                        Your trial ends in <span className="text-orange-500 font-medium">{daysLeft} days</span>.
                      </p>
                      {plans.map(plan => {
                        if (plan.id === 'pro_plan') {
                          return (
                      ...
                      <ul className="mb-8 space-y-3 flex-grow">
                        {plan.features.map((feature, index) => (
                          <li key={index} className="flex items-start gap-3">
                            <Gift className={`w-5 h-5 ${feature.includes('not included') ? 'text-gray-500' : 'text-orange-500'} shrink-0`} />
            ...
            <div className="bg-orange-500/10 p-4 rounded-lg border border-orange-500/20">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-white font-medium mb-1">Pro Plan Benefits</h4>
                ...
                <div className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-400">Update your card information</div>
                    </div>
                  </div>
                ...
                {userData?.plan === 'Pro Plan' && daysLeft !== null && (
                  <div className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-white">{userData?.plan || 'Pro Plan'}</div>
                    ...
                    {daysLeft ? (
                      <button
                        onClick={() => handlePlanClick(plans.find(p => p.id === 'pro_plan') || null)}
      ...
      {paymentModal && selectedPlan && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-[301]" onClick={() => setPaymentModal(false)}></div>
          <StripeCheckout
