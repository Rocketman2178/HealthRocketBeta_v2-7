-- Create a new version of the function with different parameter names to avoid the error
...
BEGIN
  ...
  -- Check active_contests table - use table alias to avoid parameter name conflicts
  SELECT EXISTS (
    SELECT 1 
    FROM active_contests ac
    WHERE ac.user_id = p_user_id AND ac.challenge_id = p_challenge_id
  ) INTO v_is_registered;
  
  -- If not found in active_contests, check challenges table - use table alias
  IF NOT v_is_registered THEN
    SELECT EXISTS (
      SELECT 1 
      FROM challenges c
      WHERE c.user_id = p_user_id AND c.challenge_id = p_challenge_id
    ) INTO v_is_registered;
  END IF;
...
-- Update the check_contest_eligibility_credits function to use the fixed function
...
BEGIN
  -- Use the fixed function to check if already registered
...
-- Update the register_for_contest_with_credits function to use the fixed function
