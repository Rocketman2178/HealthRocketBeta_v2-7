/*
  # Enhance All User Insights Table

  1. New Columns
    - `total_healthspan_years` (numeric) - Total +HealthSpan years across all users
    - `total_lifetime_fp` (integer) - Total lifetime FP earned across all users
    - `average_level` (numeric) - Average level of all users
    - `highest_level` (integer) - Highest level achieved by any user
    
  2. Changes
    - Update generate_all_user_insights() function to populate new columns
    - Update get_all_user_insights() function to return new columns
*/

-- Add new columns to all_user_insights table
ALTER TABLE public.all_user_insights 
  ADD COLUMN IF NOT EXISTS total_healthspan_years numeric(8,2),
  ADD COLUMN IF NOT EXISTS total_lifetime_fp integer,
  ADD COLUMN IF NOT EXISTS average_level numeric(4,2),
