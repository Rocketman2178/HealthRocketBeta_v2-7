/*
  ...
  1. Changes
    - Add missing sleep boosts to the boost_fp_values table if they don't exist
    - Fix the complete_boost function to properly handle the return value
    - Ensure proper case-insensitive category matching
...
-- Create or replace the get_boost_category function to handle case sensitivity properly
...
BEGIN
  -- Get FP value and category for the boost
  ...
  IF v_boost_category IS NULL THEN
    -- Use the get_boost_category function
    v_boost_category := get_boost_category(p_boost_id);
  END IF;
  
  -- Log the boost category detection for debugging
  INSERT INTO boost_processing_logs (
    processed_at,
    boosts_processed,
    details
    ...
    0,
    jsonb_build_object(
      'operation', 'complete_boost_category_detection',
      'boost_id', p_boost_id,
