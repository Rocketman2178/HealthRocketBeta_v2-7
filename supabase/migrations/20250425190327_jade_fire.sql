/*
  # Fix Burn Streak Increment

  1. Changes
    - Create a function to update burn streak when a boost is completed
    - Add a trigger to automatically increment burn streak on first boost of the day
    - Fix issue where burn streak wasn't incrementing properly
...
CREATE FUNCTION check_and_reset_burn_streaks()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
