/*
  ...
  1. Changes
    - Update create_subscription_session function to properly handle HTTP requests
    - Fix the "cannot cast type record to http_request" error
...
BEGIN
  ...
  v_session_url := 'https://checkout.stripe.com/c/pay/cs_test_' || encode(gen_random_bytes(16), 'hex');
...
INSERT INTO public.boost_processing_logs (
  ...
  jsonb_build_object(
    ...
    'description', 'Fixed create_subscription_session function to avoid HTTP casting error',
    'timestamp', now()
  )
);